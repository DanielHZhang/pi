...
async function walkDirectoryWithFd(
	...
	maxResults: number,
	signal: AbortSignal,
	options: { maxDepth?: number; fullPath?: boolean; pattern?: string } = {},
...
export class CombinedAutocompleteProvider implements AutocompleteProvider {
	...
	private getSlashCommandSuggestions(prefix: string, skillsOnly: boolean): AutocompleteSuggestions | null {
		...
		const items: AutocompleteItem[] = fuzzyFilter(commands, query, (command) =>
			!query.startsWith("skill:") && command.name.startsWith("skill:")
	...
	private async getBaseDirSuggestions(
		...
		return await walkDirectoryWithFd(baseDir, this.fdPath, query, 100, signal, { maxDepth: 1 });
	}
	...
	private async getFuzzyFileSuggestions(
		...
		try {
			...
			const recursiveEntries = await walkDirectoryWithFd(
				fdBaseDir,
				this.fdPath,
				fdQuery,
